		...
		do
		{
			...
			for (int x = x1; x <= x2; x++)
			{
				if ((y - 1) > originalY && !pmap[y - 1][x])
				{
					...
					if (!pmap[y - 1][randPos] && eval_move(parts[i].type, randPos, y - 1, nullptr, i))
						x = randPos;
					// Couldn't move to random position, so try the original position on the left
					else if (!eval_move(parts[i].type, x, y - 1, nullptr, i))
						continue;
...
void Simulation::clear_sim(void)
{
	...
	//Reset Cyens Toy options
	infoScreenEnabled = false;
	timeDilationEnabled = false;
	compressibleGasesEnabled = false;
	drawQuantumFields = false;

	//Clear Cyens Fields
	for (int y = 0; y < YRES; y++)
		for (int x = 0; x < XRES; x++) {
			strongForce[y][x] = v2f{ 0,0 };
			weakForce[y][x] = v2f{ 0,0 };
			electromagForce[y][x] = v2f{ 0,0 };
		}
}
...
int Simulation::try_move(int i, int x, int y, int nx, int ny)
{
	...
	e = eval_move(parts[i].type, nx, ny, &r, i);
...
int Simulation::create_part(int p, int x, int y, int t, int v)
{
	...
	if (p == -1)//creating from anything but brush
	{
		...
		if (pmap[y][x] ? (eval_move(t, x, y, NULL, p) != 2) : (bmap[y / CELL][x / CELL] && eval_move(t, x, y, NULL, p) == 0))
		{
			return -1;
		}
		if (pfree == -1)
...
void Simulation::GetGravityField(int x, int y, float particleGrav, float newtonGrav, float& pGravX, float& pGravY)
{
	...
	case 0: //normal, vertical gravity
		...
		break;
	case 1: //no gravity
	case 3: //Cyens Toy local gravity
		break;
...
void Simulation::addpressure(float k,int i, int y, int x){
	if(!bmap[y][x]){
		pv[y][x] += k;
	}
	//lets not deal with storepressure
	/*else{
		storepressure[i] += k;
	}*/
}

//Cyens Toy Field Update
void Simulation::UpdateCyensFields() {
	float sfFade = 0.975f;
	for (int y = 0; y < YRES; y++) {
		for (int x = 0; x < XRES; x++) {
			v2f* sf = &strongForce[y][x];
			if (mag(*sf)) {
				sf->x *= sfFade;
				sf->y *= sfFade;
				if (sf->x >= -0.009f && sf->x <= 0.009f)sf->x = 0;
				if (sf->y >= -0.009f && sf->y <= 0.009f)sf->y = 0;
			}
		}
	}
}
...
{
	int i, j, x, y, t, nx, ny, r, surround_space, s, rt, nt, ctypep, tmpp;
	float mv, dx, dy, nrx, nry, dp, ctemph, ctempl, gravtot;
	int fin_x, fin_y, clear_x, clear_y, stagnant, type3;
	float fin_xf, fin_yf, clear_xf, clear_yf, diffx, diffy;
	...
	for (i = start; i <= end && i <= parts_lastActiveIndex; i++)
		if (parts[i].type)
		{
			...
			if (canTick && (!(parts[i].tmp2 < 1000) || !(elements[t].Properties&TYPE_SOLID))) {
			...
			//Cyens Toy: Compression starts at this pressure
			int minCompress = 50;
			...
			if (!legacy_enable)
			{
...
#endif
				{
...
#ifdef REALISTIC
						if(elements[t].pressureresistance > 0 && parts[i].tmp2 == 1000){
								parts[i].tmp2 = 0;
						}
						float dbt = ctempl - pt;
					...
					//// variable boiling point for PT_LIQUID and PT_GASEOUS
					if(t == PT_LAVA && elements[parts[i].ctype].GasTransition != NT){
						ctemph = pt - 2.0f*pv[y/CELL][x/CELL];
					}else if(t == PT_GASEOUS && elements[parts[i].ctype].GasTransition != NT){
						ctempl = pt - 2.0f*pv[y/CELL][x/CELL];
					} else if(elements[t].Liquidtransition > 0){
						ctempl = pt - 2.0f*pv[y / CELL][x / CELL];
					} else if(t == PT_LIQUID && elements[parts[i].ctype].Liquidtransition > 0){
						ctemph = pt - 2.0f*pv[y / CELL][x / CELL];
					}
			...
			if(parts[i].tmp2 < 1000 && elements[t].pressureresistance > 0 && elements[t].Properties&TYPE_SOLID){
				...
				for(int yy = -1; yy <= 1; yy++){
					for(int xx = -1; xx <= 1; xx++){
						type2 = pmap[y + yy][x + xx];
						if(elements[TYP(type2)].Properties&TYPE_SOLID && parts[ID(type2)].tmp2 < 1000){
							parts[i].tmp2 += elements[TYP(type2)].pressureresistance;
						}
					}
				}
			...
			//Cyens Toy particle update
			ClampOrganic(&parts[i]);

			//Compressible Gases
			if (compressibleGasesEnabled && (elements[t].Properties & TYPE_GAS)) {
				...
				for (int xx = -1; xx <= 1; xx++) {
					...
					if (xOffs < 0 || xOffs >= XRES) continue;
					for (int yy = -1; yy <= 1; yy++) {
						int yOffs = parts[i].y + yy;
						if (yOffs < 0 || yOffs >= YRES) continue;
						...
						if (i == ID(adj) || parts[ID(adj)].type != parts[i].type) continue;
						if (parts[i].gasCompression < 5 && !parts[ID(adj)].gasCompression && pressure > minCompress + parts[i].gasCompression * 25) {
							int g = parts[ID(adj)].gasCompression;
							parts[i].gasCompression += !g ? 1 : g;
							pv[y / CELL][x / CELL] -= 5;
							kill_part(ID(adj));
						}
					}
				...
				//Decompression
				if (parts[i].gasCompression && (decompX + decompY > 0) && pressure <= minCompress + parts[i].gasCompression * 25) {
					create_part(-1, decompX, decompY, parts[i].type);
					parts[i].gasCompression--;
					pv[y / CELL][x / CELL] += 5;
				}
			}
		...
		killed:
			...
			else
			{
				...
				while (1)
				{
					...
					int eval = eval_move(t, fin_x, fin_y, NULL, i);
			...
			else if (elements[t].Properties & TYPE_ENERGY)
			{
				if (t == PT_PHOT)
				{
					...
					if (eval_move(PT_PHOT, fin_x, fin_y, NULL, i))
					{
			...
			else if (elements[t].Falldown==0 || ((parts[i].tmp2 < 1000) && (elements[t].Properties&TYPE_SOLID)))
			{

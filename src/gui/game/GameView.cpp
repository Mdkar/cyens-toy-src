	...
	else if (showHud)
	{
		...
		if (type)
		{
			...
			if (showDebug)
			{
				...
				else if (type == PT_NCTD) {
					sampleInfo << c->nucleotideName(sample.particle.life, sample.particle.tmp);
				}
				else if (type == PT_PHOS) {
					if (sample.particle.tmp == PHOS_RED)
						sampleInfo << "Red";
					else
						sampleInfo << "White";

					sampleInfo << " Phosphorus";
				}
				else if (type == PT_SUGR) {
					switch (sample.particle.tmp) {
					case SUGR_GLUCOSE:
						sampleInfo << "Glucose";
						break;
					case SUGR_RIBOSE:
						sampleInfo << "Ribose";
						break;
					case SUGR_DEOXYRIBOSE:
						sampleInfo << "Deoxyribose";
						break;
					}
					sampleInfo << " Sugar";
				}
				else if (type == PT_WSTE) {
					switch (sample.particle.tmp2) {
					case WSTE_SDM_ACETATE:
						sampleInfo << "Sodium Acetate";
						break;
					case WSTE_TRSDM_PHOSPHATE:
						sampleInfo << "Trisodium Phosphate";
						break;
					}
				}
				else if (type == PT_QARK) {
					switch (sample.particle.tmp) {
					case QARK_UP:
						sampleInfo << "Up";
						break;
				...
				else if (type == PT_WBSN) {
					if (sample.particle.tmp)sampleInfo << "W+ Boson";
					else sampleInfo << "W- Boson";
				}
				else
				{
